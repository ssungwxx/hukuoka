  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-8 pb-24">
      ...
      <section id="schedule" className="mb-12">
        ...
        <div className="space-y-6">
          ...
          <DaySchedule
            ...
            title="맥주공장 & 여유 쇼핑"
            transportCost="~¥2,250"
            mapUrl="https://www.google.com/maps/dir/博多駅/西鉄福岡天神駅/太刀洗駅/キリンビール福岡工場/太刀洗駅/博多一双+博多駅東本店/ドン・キホーテ中洲店/かわ屋+薬院+福岡"
            items={day3Items}
      ...
      <section id="budget" className="mb-12">
        ...
        <BudgetSection
          total={{ label: "합계 (숙소 제외)", amount: "~¥31,500", detail: "약 28만원 (환율 ¥100=₩900 기준)" }}
          items={[
            { label: "교통비 총합", amount: "~¥8,000", detail: "공항↔시내, 시내이동, 기타큐슈 왕복 포함", icon: "🚇" },
            { label: "식비 총합", amount: "~¥20,500", detail: "9끼 식사 (야타이~가라토시장 초밥까지)", icon: "🍽️" },
            { label: "액티비티", amount: "~¥3,000", detail: "클라이밍 + 맥주공장 (¥500)", icon: "🎯" },
          ]}
        />
      </section>
...
const day1Items: ScheduleItem[] = [
  {
    ...
    detail: "인천 15:20 출발 · 비행 약 1시간 40분 · 입국심사·수하물 약 30~45분",
...
const day3Items: ScheduleItem[] = [
  ...
  },
  {
    ...
    detail: "투어 종료 후 여유 있게 출발 · 타치아라이→오고리(¥280) → 니시테츠→텐진(¥530) · 약 1시간~1시간 30분 소요",
    cost: "¥810",
  },
  {
    time: "13:30",
...
const day4Items: ScheduleItem[] = [
  {
    ...
    emoji: "☕",
    text: "여유 오전 / 카페 휴식",
    detail: "전날 맥주공장 이동이 많았으므로 늦잠·여유롭게 준비. 호텔 주변 카페에서 브런치도 OK",
  },
  {
    time: "12:00",
    emoji: "🍜",
  ...
  },
  {
    ...
    emoji: "🚇",
    text: "베어핸즈 클라이밍으로 이동",
    detail: "하카타역 → 텐진미나미역(지하철) → 나나쿠마역 (나나쿠마선, 총 25분)",
    cost: "¥300",
  },
  {
    time: "13:30",
...
const transportRoutes: TransportRoute[] = [
  ...
  { from: "하카타역", to: "야쿠인역", method: "지하철 공항선→텐진 환승 or 도보", time: "15~20분", cost: "~¥260" },
  { from: "텐진역", to: "텐진미나미역", method: "도보 (환승)", time: "3~5분", cost: "무료" },
  { from: "니시공원", to: "텐진", method: "니시테츠 버스", time: "약 15분", cost: "~¥230" },
  { from: "마이즈루공원", to: "니시공원", method: "도보", time: "25~30분", cost: "무료" },
];
